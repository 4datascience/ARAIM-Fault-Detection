function GridInDegrees = GridSphere(numPoints)
% Generates a geodesic grid, a set of nearly evenly spaced points on a sphere.
% Points are sorted in order of increasing latitude.  Ties between latitudes
% that differ only by floating point errors are broken by a secondary sort in
% order of increasing longitude.  Tesselates a sphere with triangles.  Starts
% with an icosahedron circumscribed by a unit sphere.  Divides the triangles
% into 4 new equal triangles.  Projects the new vertices onto the unit sphere.
% Repeats until reaching the desired resolution.  Refer to
...
%    usage: [latGridInDegrees, longGridInDegrees] = GridSphere(numPoints)
    ...
    % triangle.  Columns represent x, y, and z coordinates.  Depth represents
    % the A, B, and C triangle vertices.  Ensure that each vertex is an A, B,
    % and C vertex in at least one triangle.
    ...
    GridInDegrees = [latGridInDegrees, longGridInDegrees];
