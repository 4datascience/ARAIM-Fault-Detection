...
function [sigma, sigma_ss, bias, bias_ss, s1vec, s2vec, s3vec] = compute_subset_solutions(...
             G, sigpr2_int, sigpr2_acc, nom_bias_int, nom_bias_acc, subsets)
...
%  subsets is a row of ones corresponding to the no-failure mode
...
for i=1:nsets
    ...
    S = compute_s_coefficients(G, W, GtW, invcov0, indb);
    s1vec(i,:)= S(1,:);
    s2vec(i,:)= S(2,:);
    s3vec(i,:)= S(3,:);
end
